...
> **注意:本文件 §一–§五 描述的是 v2.5 的任务,已经完成且部分决定已被 v3.0 推翻。**
> 当前的产品定义以 `vibecoding-retro-PRD.md`(v3.0)为准,不要按下面的旧任务清单执行。
...
- **已完成(v3.0 做减法)**——起因是产品所有者本人看不懂报告里的 `KB-01`/`T1`/`L 档`,并指出架构紊乱:
  - **删掉分类学**:T1–T6 / J1–J4 编号、S/M/L 档位、KB-nn 编号全部消失。观察能力保留,改用现象命名("规格来得太晚")
  - **删掉状态机**:缺口台账(观察→确认3次→结构性→毕业→否决2次停检)、待实践四态、建议实践率、词汇台账聚类、五维评分卡。理由:要积累三五次才产出第一个结论,通用用户到不了
  - 建议改成**纯追加日志**(`notes.json`),不要求用户回来更新状态
  - `effect` 改为按日期切分,不再依赖 tips;`complexity` 改名 `scale`(small/medium/large),旧记录自动兼容
  - references 重组为三份白话文档:`waste-patterns.md`(硬阈值)、`observation-patterns.md`(九类现象)、`playbook.md`(按现象索引的改法)
  - SKILL.md 与报告模板加了**正反例对照表和自查清单**,把"说人话"变成可检查的规则
  - 边界新增两条:不给对话效率范围之外的建议(产品规划/职业发展);不把某次复盘的结论写进 skill 自身文件
  - `ledger.py` 702→500 行,references 411→353 行;81 个单测全绿
- **下一步**:① 用新格式实跑一次复盘,检验报告是否真的说人话 ② 若 `compare` 长期用不上(积累太慢),考虑一并删掉 ③ 校准浪费阈值与密钥扫描误报率
- **残留状态**:用户旧台账已归档为 `~/.claude/vibecoding-retro/gap-ledger.legacy-v2.json`(新代码不读),`sessions.jsonl` 5 条原样保留且被新代码正确识别为 legacy 规模
